...
L'algoritmo del simplesso si comporta in modo decrescete e ad ogni iterazione genera una nuova BFS. Se ad ogni iterazione $\Delta z < 0$ vuol dire che avremo come \hl{casi sfavorevoli}:

\begin{itemize}
	\item $\min < 0$: l'algoritmo visita tutte le BFS
	\item $\min = 0$: uno dei numeratori è nullo
\end{itemize}


se il minimo dei rapporti è = 0 icoe x\_s puo essere 0 se esiste un certo temine noto $\overline{b}_i = 0$ per $\overline{a}_{is} > 0$

es è / 0 allora non c'e un miglioramenteo nel passare da un BFS corrente ad un altra allora quidni la sluzione succesiva ha la stessa soluzione della precendte ma poi dopo posso comeunque trovare delle soluzioni migliorative

img

posso avere un caso sfortunato detto cycling dove parto da una soluzione, arriva ad un altra dove c'e questo problema dove no c'e una variazione di z, questo problema si verifica anche oer le iterazioni successive. opiche l'algoritmo è derministico, contineuro a generare la stessa sequenza all'inifnito.
quindi l'algoritmo non converge. questo aspetto potrebbe produrre il cycling quindi la mancata convergenza dell'algoritmo.
...
abbiamo uan variabile in base = 0 quidni abiamo una soluzione degenere
...
abbiamo un'altra soluzione degenere quidni ci troviamo su un plateau. dal punti di vista grafico abbiamo che x\_1 rimane 0 e quidi rimaniamo sempre nel punto 0,0 quisto perche c'e una differenza enorme tra O e A dato che ha il numero minimo ed indispenzabile id vincoli. inveceO è dato da un numero ridondadnte  e superfluo di vincoli. abbiamo quidni 2 rappresentaizoni in termini di soluzioni di base ammissibili.
...
prendendo come perno un second ovalore di $x_2$ avremo un miglioramnte:

tab

quidni con $\Delta z = - 20$