...
il test si ottimalità fallisce e quindi possiamo avere una soluzione migliore e quindi applico l'euristica del valore di costo ridotto. Sceglo a caso $x_s = x_1$.

Prendiamo i valori di $x_1$ e in base al valore unitario presente in $x_3$ e $x_4$ allora avremo:
$$\min (\frac{10}{2}, \frac{10}{1}) = 5$$

non posso avere che $x_3 > 5$ dato che si deve fermare lì perchè $x_3$ diventa negativa e quindi non ammissibile dato che superiamo in punto $B$.

Allora dato che $x_3 = 0$ e $x_1$ prende il suo posto avremo che le variabili di base saranno:
$$J_B = <x_1, x_4>$$
...
allora abbiamo che l asoluzione di base ammissibile BFS è:
...
il test di ottimalità fallisce data la presenza del $-5$, quindi la variabile entrante è $x_2 = x_s$ quindi:
$$x_r = \text{argmin} (\frac{5}{0.5}, \frac{5}{1.5}) = x_4$$

il che corrisponde allo stare nel punto $B$ dove $x_2 = 0$, quindi ci spostiamo in direzione di $A = (> 0, > 0, 0, 0)$. avremo allora:
$$J_B = <x_1, x_2>$$

dato che $x_2$ è entrante ed $x_4$ uscente.
...
allora abbiamo ottenuto una soluzione ottima.




Individuiamo soluzioni ottime multiple. come le individuiamo? Per poter esaminare questo caso partiamo col dire che esiste una sluzione ottima se 

$$\overline{c_j} \geq 0$$

essitono soluzioni multiple se esiste $j^{*} \in J_N$ e quidni ha i coeficient id costo ridotti maggiori di zero sium oppure se i corff delle varibiali non di base sono > 0 e quidni almeno uno sia = 0. allora se faccio il solito ragionamento: se prendo una variabile $x_{j*} = 0 \to\ > 0$ allora avrò che:
...
quindi il test di ottimalità sulle variabili fuorni base $J_N = <x_2, x_3>$ è OK. ma non è una soluione unica dato che abbiamo $0$ per la variabile $x_2$ che andrà perturbata assegnandole un valore $> 0$. allora la varaizione della funzione obiettivo per una perturbazopne è $\overline{c}_{j*}x_{j*}$ con $\overline{c}_{j*}=0$ e $x_{j*} > 0$. allora abbiamo che lungpo tutto lo spigolo $AB$ avremo un costo minimo.

infatti forzando Pivot in $x_2$ esce $x_4$:
...
quindi abbiamo generato i due punti estremi che sono A e B allor avremo infinite soluzioni ottime dato che ci muoveremo nel segmento AB