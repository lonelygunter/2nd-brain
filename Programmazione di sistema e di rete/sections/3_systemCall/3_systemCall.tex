...
\begin{itemize}
	...
	\item if 1: gestisce un segnale di errore
		...
		\begin{itemize}
			\item \hl{signal}: gli diciamo che se arriva il segnale SIGINT allora eseguire la nostra funzione sig\_int
...
Potremo \hl{inviare i segnali tramite il comando "kill"}. Per esempio per SIGINT: $\text{ctrl}\ C$ oppure per SIGQUIT: $\text{ctrl}\ \backslash $.
